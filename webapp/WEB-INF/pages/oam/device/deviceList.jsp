...
var oTable;
 ...
 function add(){
	 var index = layer.open({
	        ...
	        area : ['800px' , ''],
	        content:basePath+"device/toAddPage?page=devEdit",
			btn:['提交','取消'],
 ...
 function alter(obj){
	 ...
	 if(!data.id){
   	...
   	var index = layer.open({
			...
			shadeClose: false, //点击遮罩关闭层
	        area : ['800px' , ''],
	        content:basePath+"device/alterData?page=devEdit&id="+data.id,
			btn:['提交','取消'],
			yes:function(index,layero){	
				 var pass =  window[layero.find('iframe')[0]['name']].valid();
				 if(!pass) return false;
 ...
 function detail(obj){
	 ...
	 if(!data.id){
   	...
   	var index = layer.open({
			...
			shadeClose: false, //点击遮罩关闭层
	        area : ['800px' , ''],
	        content:basePath+"device/alterData?opt=detail&page=devEdit&id="+data.id,
			btn:['关闭']
	    });  
   	layer.full(index); 
 }
