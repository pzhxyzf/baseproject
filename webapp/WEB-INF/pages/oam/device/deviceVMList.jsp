...
var oTable;
		...
		//初始化列表
	 	oTable =  $('#oTable').DataTable( {
	        ...
	        "columns": [
	            ...
	            {"title": "设备序号",  "data": "host.serialNo", "defaultContent":""},
	            {"title": "固定资产编号", "data": "host.fixedNo", "defaultContent":"" },
 ...
 function add(){
	 var index = layer.open({
	        ...
	        area : ['800px' , ''],
	        content:basePath+"device/toAddPage?page=vmEdit",
			btn:['提交','取消'],
 ...
 function alter(obj){
	 ...
	 if(!data.id){
   	...
   	var index = layer.open({
			...
			shadeClose: false, //点击遮罩关闭层
	        area : ['800px' , ''],
	        content:basePath+"device/alterData?page=vmEdit&id="+data.id,
			btn:['提交','取消'],
			yes:function(index,layero){	
				 var pass =  window[layero.find('iframe')[0]['name']].valid();
				 if(!pass) return false;
 ...
 function detail(obj){
	 ...
	 if(!data.id){
   	...
   	var index = layer.open({
			...
			shadeClose: false, //点击遮罩关闭层
	        area : ['800px' , ''],
	        content:basePath+"device/alterData?opt=detail&page=vmEdit&id="+data.id,
			btn:['关闭']
	    });  
   	layer.full(index); 
 }
